...
import { X, User, Mail, Shield, CheckCircle2, AlertTriangle, Loader2, Trash2 } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-200">
        ...
        {/* Content */}
        <form onSubmit={handleUpdate} className="p-6 space-y-6 font-sans">
          {error && (
          ...
          {confirmDelete ? (
            <div className="bg-red-50/60 border border-red-200 rounded-xl p-5 text-center space-y-3">
              <div className="size-10 rounded-full bg-red-100 text-red-600 mx-auto flex items-center justify-center">
                <Trash2 size={18} />
              </div>
              <div>
                <h4 className="font-bold text-slate-900 text-sm">Delete Account "{user.name}"?</h4>
                <p className="text-xs text-slate-500 mt-1">
                  This action is permanent and will remove all associated user data and credentials.
                </p>
              </div>
              <div className="flex items-center gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2 border border-slate-200 bg-white rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
            ...
            <>
              ...
              <div className="space-y-4">
                <label className="grid gap-2 text-sm font-semibold text-slate-700">
                  ...
                  <select
                    ...
                    className="h-11 w-full px-3 border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:border-[#3b338c] text-sm text-slate-750 font-normal outline-none transition-colors cursor-pointer"
                  >
              ...
              <div className="flex justify-between items-center pt-2">
                <button
                  ...
                  className="h-10 px-4 bg-red-50 text-red-650 hover:bg-red-100 border border-red-200/50 font-semibold rounded-lg text-xs transition-colors cursor-pointer flex items-center gap-1.5"
                >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="h-10 px-5 bg-[#3b338c] text-white font-semibold rounded-lg text-xs hover:bg-violet-950 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 shadow-xs"
                  >
